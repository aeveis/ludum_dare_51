...
class MultiSprite extends FlxSprite
{
	...
	public function spriteUpdate(sprite:SpriteProperty, elapsed:Float)
	{
		if (moves) //update sprite motion
		{
			...
			var velocityDelta = 0.5 * (FlxVelocity.computeVelocity(sprite.angularVelocity, angularAcceleration, angularDrag, maxAngular, elapsed) - sprite.angularVelocity);
	...
	override public function draw()
	{
		...
		for (camera in cameras)
		{
			...
			for (sprite in sprites)
			{
				...
				if (isOnScreenMulti(sprite) && sprite.active && sprite.alpha != 0)
				{
	...
	function drawSimpleMulti(sprite:SpriteProperty, camera:FlxCamera):Void
	{
	...
	function drawComplexMulti(sprite:SpriteProperty, camera:FlxCamera):Void
	{
	 ...
	 */
	public function isSimpleRenderMulti(sprite:SpriteProperty, ?camera:FlxCamera):Bool
	{ 
		if (FlxG.renderTile)
			return false;
